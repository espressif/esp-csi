...
{
    if (!g_wifi_radar_data.status_enable) {
        return;
    }
    ...
    if (g_wifi_radar_data.threshold_adjust) {
        if (!s_threshold_adjust_count) {
            g_wifi_radar_data.move_relative_threshold = 0;
    ...
    if (amplitude_std > g_wifi_radar_data.move_absolute_threshold || trigger_relative_flag) {
        ESP_LOGW(TAG, "Someone is moving");
        light_driver_set_switch(true);

        g_wifi_radar_data.awake_count++;
        g_wifi_radar_data.room_status = true;
...
void app_main(void)
{
    ...
    if (esp_qcloud_storage_get("move_absolute", &g_wifi_radar_data.move_absolute_threshold, sizeof(float)) != ESP_OK) {
        g_wifi_radar_data.move_absolute_threshold = 0.2;
    }

    if (esp_qcloud_storage_get("move_relative", &g_wifi_radar_data.move_relative_threshold, sizeof(float)) != ESP_OK) {
        g_wifi_radar_data.move_relative_threshold = 1.5;
    }
    ...
    if (esp_qcloud_storage_get("filter_mac", radar_config.filter_mac,
        ...
        ESP_LOGI(TAG, "Using " MACSTR " as the source of csi data", MAC2STR(radar_config.filter_mac));
    }
